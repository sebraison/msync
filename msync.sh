...
function do_help
{
	...
	echo "Agregate multiple cloud-synchronized storages into a single mount point.

Because we massively rely on rclone, you can use any provider it supports. We
...
This tool MUST be run as root beacuse aufs create root-owned metadata files in
each directory used as backend, and we need to access them to synchronise.

Syntax : $0 <command> <argument1> <...>
...
With command :
    ...
    check <install dir>
        ...
        base requierments (rclone, iwatch, aufs module). If <install dir> is
    ...
    import <install dir>
        ...
        Retreive cloud data afer a fresh (re) install, MUST be run before the
        start command or you will overwrite all saved data on cloud. Note you
        may have to fix permissions beause cloud storages does not preserve
...
msync logs many things in the <install dir>/msync.log file.

Please refer to homepage for full documentation.
"
}
